...
- Фоновые уведомления о новых сериях переведены на JobScheduler + AlarmManager fallback, с восстановлением после перезагрузки/обновления и статусом последней проверки в профиле.
...
- Java-only сборка сохранена: активный модуль только `:app`, версия `4.12.8` / `51`.
...
Проверенный APK: `apk-output/YORU-4.12.8-release.apk`.
