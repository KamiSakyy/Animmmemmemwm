...
public final class YoruApp extends Application {
    ...
    @Override public void onCreate(){super.onCreate();instance=this;original=true;store=new SecureStore(this);cache=new YoruCache(this);traffic=new TrafficMeter(this,store);mediaCache=new MediaCache(this);api=new ApiRepository(this);images=new ImageLoader(this);channels();main.postDelayed(this::afterFirstFrame,350);}
    private void afterFirstFrame(){ui.execute(()->{boolean ok=AppSecurity.original(this);main.post(()->original=ok);});main.postDelayed(()->{try{WebView.setWebContentsDebuggingEnabled(false);}catch(Exception ignored){}},250);main.postDelayed(()->EpisodeUpdateReceiver.schedule(this),700);main.postDelayed(()->EpisodeUpdateReceiver.checkNow(this),18000);main.postDelayed(()->io.execute(()->{try{api.refreshProtection(false);}catch(Exception ignored){}}),12000);warmStartup();}
    public void warmStartup(){if(warmed)return;warmed=true;calendarTodayCount=cache==null?0:cache.todayScheduleCount();ui.execute(()->{try{if(cache!=null)cache.trimNow();}catch(Exception ignored){}try{downloads().refreshIndexAsync();}catch(Exception ignored){}try{api.prefetchQuickDetails(api.seed(),10);}catch(Exception ignored){}});discovery.execute(()->{try{long at=store.calendarCacheAt();if(System.currentTimeMillis()-at<25*60*1000L&&calendarTodayCount>0)return;ArrayList<ApiRepository.AiringItem> rows=api.airingSchedule(21,store.favorites());JSONArray json=ApiRepository.airingJson(rows);if(cache!=null)cache.schedule(json);store.calendarCache(json);calendarTodayCount=ApiRepository.todayCount(rows);}catch(Exception ignored){}});}
