...
public final class YoruApp extends Application {
    ...
    private static ExecutorService pool(String name,int core,int max,int priority){ThreadPoolExecutor e=new TaskQueue.Executor(core,max,20L,TimeUnit.SECONDS,new LinkedBlockingQueue<>(256),r->{Thread t=new Thread(r,name);t.setPriority(priority);return t;},new TaskQueue.Policy());e.allowCoreThreadTimeOut(true);return e;}
