...
public final class NativeSources {
    ...
    public Anime.Page catalog(String source,String search,int page)throws Exception{
        if(!source.equals("animedia"))throw new IOException("Неизвестный источник");String base="https://amd.online";String html;
    ...
    public Anime details(Anime base,boolean episodes)throws Exception{
        String host="https://amd.online";String url=base.alias.startsWith("https://")?base.alias:absolute(host,base.alias);if(url.isEmpty()||!new URL(url).getHost().equals(new URL(host).getHost()))throw new IOException("Откройте аниме из каталога источника");Document doc=Jsoup.parse(api.document(url),url);Anime a=Anime.from(base.json());a.alias=url;
