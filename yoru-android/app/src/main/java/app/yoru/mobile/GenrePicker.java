...
final class GenrePicker extends LinearLayout {
    ...
    private void open(){
        ...
        list.setOnItemClickListener((parent,view,index,id)->{if(index>=visible.size())return;selected=visible.get(index)[0];selectedTitle=visible.get(index)[1];button.setText(selectedTitle);dialog.dismiss();});
    }
}
