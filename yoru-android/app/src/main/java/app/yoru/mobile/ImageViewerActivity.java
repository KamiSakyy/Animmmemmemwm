...
public final class ImageViewerActivity extends Activity {
  ...
  public void onCreate(Bundle b) {
    ...
    setContentView(root);
    image = new ZoomImage(this);
    image.setBackgroundColor(Color.BLACK);
    root.addView(image, new FrameLayout.LayoutParams(-1, -1));
    String url = getIntent().getStringExtra("url"),
      title = getIntent().getStringExtra("title");
    YoruApp.app().images.load(image, url == null ? "" : url, "image-viewer");
    ...
    TextView name = Ui.text(
      this,
      title == null || title.isEmpty() ? "Скриншот" : title,
      14,
      Ui.TEXT,
      true
    );
    ...
    hint.setBackgroundColor(0x44000000);
    root.addView(hint, new FrameLayout.LayoutParams(-1, -2, Gravity.BOTTOM));
  }
  ...
  public static final class ZoomImage extends ImageView {
    ...
    public ZoomImage(android.content.Context c) {
      super(c);
      ...
      scale = new ScaleGestureDetector(
        ...
        new ScaleGestureDetector.SimpleOnScaleGestureListener() {
          ...
          public boolean onScale(ScaleGestureDetector d) {
            ...
            float max = Math.max(5f, minScale * 5f);
            float next = Math.max(minScale, Math.min(max, current * factor));
            float real = current <= 0 ? 1f : next / current;
    ...
    private float currentScale() {
      ...
      return values[Matrix.MSCALE_X] <= 0 ? 1f : values[Matrix.MSCALE_X];
    }
    ...
    public boolean onTouchEvent(MotionEvent e) {
      ...
      switch (e.getActionMasked()) {
        ...
        case MotionEvent.ACTION_CANCEL:
          drag = false;
          if (
            e.getEventTime() - e.getDownTime() < 180 &&
            currentScale() > minScale + .05f
          ) reset();
          return true;
      }
      return true;
