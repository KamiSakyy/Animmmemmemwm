...
public final class TrafficMeter {
    ...
    public synchronized void sample(){long rx=TrafficStats.getUidRxBytes(android.os.Process.myUid()),tx=TrafficStats.getUidTxBytes(android.os.Process.myUid());long now=SystemClock.elapsedRealtime();if(rx<0||tx<0){available=false;return;}available=true;long total=rx+tx;if(lastTotal>=0&&total>=lastTotal&&now>=lastElapsed){long delta=total-lastTotal;int kind=now-lastElapsed>120000?2:lastKind;if(kind==0)mobile+=delta;else if(kind==1)wifi+=delta;else other+=delta;}lastTotal=total;lastElapsed=now;lastKind=networkKind();persist();}
    private void persist(){try{store.traffic(new JSONObject().put("mobile",mobile).put("wifi",wifi).put("other",other).put("since",since).put("lastTotal",lastTotal).put("lastElapsed",lastElapsed).put("lastKind",lastKind));}catch(Exception ignored){}}
    public synchronized JSONObject snapshot(){sample();try{return new JSONObject().put("mobile",mobile).put("wifi",wifi).put("other",other).put("since",since).put("supported",available);}catch(Exception e){return new JSONObject();}}
    public synchronized void reset(){mobile=wifi=other=0;since=System.currentTimeMillis();lastTotal=-1;sample();}
}
