...
public final class DownloadHub {
    ...
    private void enqueue(Anime catalog,Anime source,Anime.Episode ep,int quality,String voice,String mime,String replacementId){if(!Anime.valid(catalog)||!Anime.valid(source)||ep==null||ep.future||!EpisodeRules.allowsNumber(catalog,ep.number)||EpisodeRules.conflicts(catalog,source)){Ui.toast(context,"Выбранная серия недоступна для этого аниме");return;}String url=ep.streams.get(quality);if(url==null||ApiRepository.safeUrl(url).isEmpty()){Ui.toast(context,"Серия пока не подготовлена для загрузки");return;}String id=replacementId.isEmpty()?idFor(source,ep,quality):replacementId;Download exists=get(id);if(exists!=null&&exists.state==Download.STATE_COMPLETED){Ui.toast(context,"Эта серия уже скачана");return;}if(exists!=null&&exists.state!=Download.STATE_FAILED&&exists.state!=Download.STATE_STOPPED&&exists.state!=Download.STATE_REMOVING){Ui.toast(context,"Эта серия уже в очереди");return;}if(!preparing.add(id)){Ui.toast(context,"Серия уже подготавливается");return;}if(context.getFilesDir().getUsableSpace()<100L*1024*1024){preparing.remove(id);Ui.toast(context,"Недостаточно свободного места");return;}
        Ui.toast(context,"Подготавливаем серию к скачиванию…");try{JSONObject meta=new JSONObject().put("anime",catalog.json()).put("source",source.json()).put("episode",ep.number).put("episodeId",ep.id).put("title",ep.name).put("voice",voice==null?"":voice).put("episodePoster",episodePoster(catalog,ep)).put("quality",quality).put("created",System.currentTimeMillis());MediaItem item=item(url,mime);TrackSelectionParameters selection=DownloadTracks.parameters(context,quality);DownloadHelper helper=DownloadHelper.forMediaItem(item,selection,new DefaultRenderersFactory(context),caches.http());helper.prepare(new DownloadHelper.Callback(){@Override public void onPrepared(DownloadHelper h){try{DownloadTracks.requireSelectedQuality(h,quality);DownloadRequest request=h.getDownloadRequest(id,meta.toString().getBytes(StandardCharsets.UTF_8));Download current=get(id);if(!replacementId.isEmpty()&&(exists==null||current==null||current.updateTimeMs!=exists.updateTimeMs||current.state==Download.STATE_REMOVING))return;if(current!=null&&(current.state==Download.STATE_FAILED||current.state==Download.STATE_REMOVING)){replacement.put(id,request);DownloadService.sendRemoveDownload(context,YoruDownloadService.class,id,true);}else DownloadService.sendAddDownload(context,YoruDownloadService.class,request,true);DownloadService.sendResumeDownloads(context,YoruDownloadService.class,true);if(!source.metadataOnly())SourceEngine.record(source.source,true,0,1,1,quality);Ui.toast(context,YoruApp.app().store.wifiDownloads()&&YoruApp.app().traffic.metered()?"Добавлено. Ожидаем Wi-Fi.":"Серия добавлена в загрузки");}catch(Exception e){Ui.toast(context,"Не удалось начать загрузку. Попробуйте другой вариант.");}finally{preparing.remove(id);h.release();}}@Override public void onPrepareError(DownloadHelper h,IOException error){preparing.remove(id);h.release();if(!source.metadataOnly())SourceEngine.record(source.source,false,0,0,0,quality);Ui.toast(context,"Серию не удалось подготовить для офлайн. Попробуйте другой вариант.");}});}catch(Exception e){preparing.remove(id);if(!source.metadataOnly())SourceEngine.record(source.source,false,0,0,0,quality);Ui.toast(context,"Эта загрузка сейчас недоступна.");}}
    ...
    static MediaItem item(String uri,String mime){MediaItem item=item(uri);return MediaSize.containerMime(mime)?item.buildUpon().setMimeType(mime).build():item;}
    ...
    public void remove(String id){TaskQueue.Signal retry=retryTasks.remove(id);if(retry!=null)retry.set(true);replacement.remove(id);DownloadService.sendRemoveDownload(context,YoruDownloadService.class,id,true);}
