...
public final class CalendarScreen extends FrameLayout {
    ...
    private final androidx.swiperefreshlayout.widget.SwipeRefreshLayout pull;private final Runnable hourly=new Runnable(){public void run(){if(attached&&getWindowVisibility()==VISIBLE){if(!refreshing)load(true);postDelayed(this,3600000L);}}};
    ...
    private final Runnable clock=this::refreshClock;private long updated;
    ...
    @Override protected void onAttachedToWindow(){super.onAttachedToWindow();attached=true;resumeClock();}
    @Override protected void onDetachedFromWindow(){attached=false;removeCallbacks(hourly);removeCallbacks(clock);generation++;if(loadFuture!=null)loadFuture.cancel(true);refreshing=false;pull.setRefreshing(false);super.onDetachedFromWindow();}
    @Override protected void onWindowVisibilityChanged(int visibility){super.onWindowVisibilityChanged(visibility);if(recycler==null)return;if(visibility==VISIBLE){if(attached)resumeClock();}else{removeCallbacks(hourly);removeCallbacks(clock);generation++;if(loadFuture!=null)loadFuture.cancel(true);refreshing=false;pull.setRefreshing(false);}}
    private void resumeClock(){if(getWindowVisibility()!=VISIBLE)return;removeCallbacks(hourly);removeCallbacks(clock);long now=System.currentTimeMillis();if(!refreshing&&(updated==0||now-updated>=3600000L||starts[0]!=ScheduleClock.dayStart(now,0,zone())))load(true);postDelayed(hourly,3600000L);postDelayed(clock,60000L-now%60000L);}
    private void refreshClock(){if(!attached||getWindowVisibility()!=VISIBLE)return;long now=System.currentTimeMillis();if(starts[0]!=ScheduleClock.dayStart(now,0,zone())){if(!refreshing)load(true);}else{buildBuckets();rebuildVisible(false);adapter.publish();}removeCallbacks(clock);postDelayed(clock,60000L-now%60000L);}
    private void load(){load(true);}
    private void load(boolean force){if(!attached||getWindowVisibility()!=VISIBLE)return;int gen=++generation;if(loadFuture!=null)loadFuture.cancel(true);prepareDays();buildBuckets();refreshing=true;state="Календарь";rebuildVisible(true);adapter.publish();loadFuture=YoruApp.app().discovery.submit(()->{ArrayList<ApiRepository.AiringItem> cached=new ArrayList<>();boolean fresh=false;try{cached=ApiRepository.airingFromJson(YoruApp.app().cache.schedule(6*60*60*1000L));if(cached.isEmpty())cached=ApiRepository.airingFromJson(readLegacyCache());fresh=!cached.isEmpty()&&System.currentTimeMillis()-YoruApp.app().store.calendarCacheAt()<25*60*1000L;}catch(Exception ignored){}final ArrayList<ApiRepository.AiringItem> cacheSnapshot=new ArrayList<>(cached);if(!cacheSnapshot.isEmpty())YoruApp.app().main.post(()->{if(alive()&&gen==generation&&items.isEmpty())setItems(cacheSnapshot,true);});if(fresh&&!force){YoruApp.app().main.post(()->{if(alive()&&gen==generation){refreshing=false;pull.setRefreshing(false);if(!cacheSnapshot.isEmpty())setItems(cacheSnapshot,true);}});return;}try{ArrayList<ApiRepository.AiringItem> loaded=YoruApp.app().api.airingSchedule(DAYS,YoruApp.app().store.favorites());ScheduledDownloads.refreshDates(activity,loaded);JSONArray json=ApiRepository.airingJson(loaded);YoruApp.app().cache.schedule(json);YoruApp.app().store.calendarCache(json);YoruApp.app().calendarTodayCount=ApiRepository.todayCount(loaded);YoruApp.app().main.post(()->{if(!alive()||gen!=generation)return;refreshing=false;updated=System.currentTimeMillis();pull.setRefreshing(false);setItems(loaded,true);});}catch(Exception e){YoruApp.app().main.post(()->{if(!alive()||gen!=generation)return;refreshing=false;pull.setRefreshing(false);if(items.isEmpty()&&!cacheSnapshot.isEmpty())setItems(cacheSnapshot,true);else{state="Календарь";rebuildVisible(false);adapter.publish();}});}});if(loadFuture.isCancelled()){refreshing=false;pull.setRefreshing(false);Ui.toast(activity,"Очередь занята. Нажмите обновление позже.");}}
    public void quickShow(){prepareDays();if(items.isEmpty()||refreshing){load();return;}buildBuckets();rebuildVisible(false);adapter.publish();}
    ...
    private int exactCount(){int n=0;ArrayList<ArrayList<ApiRepository.AiringItem>> days=buckets.get(filter);if(days!=null)for(ArrayList<ApiRepository.AiringItem> d:days)for(ApiRepository.AiringItem item:d)if("Точная дата".equals(item.precision))n++;return n;}
    ...
    private void prepareDays(){long chosen=selected>=0&&selected<DAYS?starts[selected]:-1;long now=System.currentTimeMillis();java.time.ZoneId zone=zone();for(int i=0;i<DAYS;i++){starts[i]=ScheduleClock.dayStart(now,i,zone);shortDays[i]=i==0?"Сегодня":i==1?"Завтра":ScheduleClock.format(starts[i],"EEE dd.MM",zone);longDays[i]=i==0?"Сегодня":i==1?"Завтра":ScheduleClock.format(starts[i],"EEEE, dd MMMM",zone);}if(chosen>0){int day=ScheduleClock.dayIndex(starts[0],chosen,zone);selected=day>=0&&day<DAYS?day:-1;}}
    ...
    private String eventCountdown(ApiRepository.AiringItem item){
        if("Точная дата".equals(item.precision))return countdown(item.time);
        int day=ScheduleClock.dayIndex(System.currentTimeMillis(),item.time,zone());
        if(day==0)return "ожидается сегодня";
        if(day==1)return "ожидается завтра";
        return day>0?"ожидается "+ScheduleClock.format(item.time,"dd.MM",zone()):"дата выхода уточняется";
    }
    private String countdown(long t){long diff=t-System.currentTimeMillis();if(diff<=-90*60*1000)return "время выхода по расписанию прошло";if(diff<=0)return "сейчас по расписанию";long min=diff/60000,h=min/60,d=h/24;if(d>0)return "через "+d+" д "+(h%24)+" ч";if(h>0)return "через "+h+" ч "+(min%60)+" мин";return "через "+Math.max(1,min)+" мин";}
    private final class CalendarEntry {
        ...
        CalendarEntry(int type,ApiRepository.AiringItem item) {
            ...
            signature=item==null?state+"|"+filter+"|"+selected+"|"+filterCounts.toString()+"|"+Arrays.toString(shortDays)+"|"+summaryLine()+"|"+nearestTitle()+"|"+allRows.size()+"|"+visible.size():YoruBrain.title(item.anime)+"|"+item.anime.poster+"|"+item.time+"|"+item.precision+"|"+statusSuffix(item)+"|"+eventCountdown(item);
        }
    }
    ...
    private final class CalendarHolder extends RecyclerView.ViewHolder {
        ...
        void bind(CalendarEntry entry) {
            ...
            if(entry.type==1) {
                ...
                meta.setText(eventCountdown(item)+" · "+item.precision+statusSuffix(item));
