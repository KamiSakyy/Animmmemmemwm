...
public final class CalendarScreen extends FrameLayout {
    ...
    private void refreshClock(){if(!attached||getWindowVisibility()!=VISIBLE)return;long now=System.currentTimeMillis();if(starts[0]!=ScheduleClock.dayStart(now,0,zone())){if(!refreshing)load(true);}else{rebuildVisible(false);adapter.publish();}removeCallbacks(clock);postDelayed(clock,60000L-now%60000L);}
