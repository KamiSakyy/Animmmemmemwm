...
public final class CalendarScreen extends FrameLayout {
    private static final int DAYS=28,PAGE_SIZE=5;
    ...
    private void load(){int gen=++generation;if(loadFuture!=null)loadFuture.cancel(true);prepareDays();refreshing=true;state="Календарь";rebuildVisible(true);adapter.publish();loadFuture=YoruApp.app().discovery.submit(()->{ArrayList<ApiRepository.AiringItem> cached=new ArrayList<>();boolean fresh=false;try{cached=ApiRepository.airingFromJson(YoruApp.app().cache.schedule(6*60*60*1000L));if(cached.isEmpty())cached=ApiRepository.airingFromJson(readLegacyCache());fresh=!cached.isEmpty()&&System.currentTimeMillis()-YoruApp.app().store.calendarCacheAt()<25*60*1000L;}catch(Exception ignored){}final ArrayList<ApiRepository.AiringItem> cacheSnapshot=new ArrayList<>(cached);if(!cacheSnapshot.isEmpty())YoruApp.app().main.post(()->{if(alive()&&gen==generation&&items.isEmpty())setItems(cacheSnapshot,true);});if(fresh){YoruApp.app().main.post(()->{if(alive()&&gen==generation){refreshing=false;if(!cacheSnapshot.isEmpty())setItems(cacheSnapshot,true);}});return;}try{ArrayList<ApiRepository.AiringItem> loaded=YoruApp.app().api.airingSchedule(DAYS,YoruApp.app().store.favorites());JSONArray json=ApiRepository.airingJson(loaded);YoruApp.app().cache.schedule(json);YoruApp.app().store.calendarCache(json);YoruApp.app().calendarTodayCount=ApiRepository.todayCount(loaded);YoruApp.app().main.post(()->{if(!alive()||gen!=generation)return;refreshing=false;setItems(loaded,true);});}catch(Exception e){YoruApp.app().main.post(()->{if(!alive()||gen!=generation)return;refreshing=false;if(items.isEmpty()&&!cacheSnapshot.isEmpty())setItems(cacheSnapshot,true);else{state="Календарь";rebuildVisible(false);adapter.publish();}});}});if(loadFuture.isCancelled()){refreshing=false;Ui.toast(activity,"Очередь занята. Нажмите обновление позже.");}}
    ...
    private void resetPager(){allLimit=PAGE_SIZE;}
    private boolean pagedAll(){return selected<0&&"all".equals(filter);}
    private boolean showPagerFooter(){return pagedAll()&&allRows.size()>PAGE_SIZE;}
    private boolean canLoadMore(){return pagedAll()&&visible.size()<allRows.size();}
    private void rebuildVisible(boolean reset){if(reset)resetPager();allRows.clear();allRows.addAll(dayItems(selected));visible.clear();int total=allRows.size(),limit=pagedAll()?Math.min(allLimit,total):total;if(limit>0)visible.addAll(allRows.subList(0,limit));if(selected<0){state=pagedAll()&&total>PAGE_SIZE?"Все события · показано "+visible.size()+" из "+total:"Все события · "+total;}else state=longDays[Math.max(0,Math.min(DAYS-1,selected))]+" · "+total;}
    private void maybeLoadMore(){if(!canLoadMore())return;int last=layout.findLastVisibleItemPosition();if(last>=adapter.getItemCount()-3)appendNextPage();}
    private void appendNextPage(){if(!canLoadMore())return;allLimit=Math.min(allLimit+PAGE_SIZE,allRows.size());rebuildVisible(false);adapter.publish();}
    ...
    private void jumpNearest(){int index=nearestIndex();if(index<0){Ui.toast(activity,"Событий пока нет");return;}if(pagedAll()&&index>=visible.size()){allLimit=Math.min(allRows.size(),((index/PAGE_SIZE)+1)*PAGE_SIZE);rebuildVisible(false);adapter.publish();}recycler.smoothScrollToPosition(Math.max(0,index+1));}
    ...
    private final class CalendarAdapter extends RecyclerView.Adapter<CalendarHolder> {
        ...
        void publish() {
            ...
            if(showPagerFooter())next.add(new CalendarEntry(3,null));
    ...
    private final class CalendarHolder extends RecyclerView.ViewHolder {
        ...
        CalendarHolder(int type) {
            ...
            if(type==1) {
                ...
                View download=Ui.iconButton(activity,"download","Скачать после выхода",()->{
                    if(item==null)return;
                    ScheduledDownloads.choose(activity,item.anime,item.episode,item.time,new SimpleDateFormat("dd.MM.yyyy HH:mm",ru).format(new Date(item.time))+" · "+item.precision);
                ...
                Ui.space(card,10);action=Ui.button(activity,"",false,()->{
                    if(type==3)appendNextPage();
                    else if(items.isEmpty())load();
        ...
        void bind(CalendarEntry entry) {
            ...
            if(entry.type==1) {
                ...
                title.setText(canLoadMore()?"Показано "+visible.size()+" из "+allRows.size():"Показаны все "+allRows.size()+" событий");
                line.setVisibility(View.GONE);action.setText("Показать ещё");action.setVisibility(canLoadMore()?View.VISIBLE:View.GONE);
            }
        }
    }
