...
final class MediaSize {
    ...
    static Info probeInfo(String raw){
        String url=ApiRepository.safeUrl(raw);Info unknown=new Info(-1,"");if(url.isEmpty()||manifest(url))return unknown;
        Info found=unknown;
        for(int attempt=0;attempt<2;attempt++){
            ...
            try{
                ...
                String finalUrl=connection.getURL().toString();if(manifest(finalUrl))return unknown;String encoding=connection.getContentEncoding();if(encoding!=null&&!encoding.equalsIgnoreCase("identity"))continue;
                String type=connection.getContentType();String mime=type==null?"":type.toLowerCase(Locale.ROOT).split(";",2)[0].trim();if(mime.contains("mpegurl")||mime.equals("application/dash+xml"))return unknown;boolean container=containerMime(mime);
