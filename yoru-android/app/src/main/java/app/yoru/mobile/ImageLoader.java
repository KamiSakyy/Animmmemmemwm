...
public final class ImageLoader {
    ...
    public ImageLoader(Context c){context=c.getApplicationContext();pool.allowCoreThreadTimeOut(true);deleteOldDiskCache();ensurePrivateDir();}
    public void clear(){generation++;memory.evictAll();synchronized(waitLock){waiters.clear();}erase(privateDir());ensurePrivateDir();}
    ...
    private Bitmap decodeFile(File f){try{byte[] data=readBytes(new BufferedInputStream(new FileInputStream(f)),12*1024*1024);return decodeBytes(data);}catch(Exception e){return null;}}
    private Bitmap decodeBytes(byte[] data){if(data==null||data.length<64)return null;BitmapFactory.Options bounds=new BitmapFactory.Options();bounds.inJustDecodeBounds=true;BitmapFactory.decodeByteArray(data,0,data.length,bounds);BitmapFactory.Options o=decodeOptions(false);Bitmap reuseBitmap=reuseFor(bounds.outWidth,bounds.outHeight);if(reuseBitmap!=null)o.inBitmap=reuseBitmap;try{return BitmapFactory.decodeByteArray(data,0,data.length,o);}catch(IllegalArgumentException badReuse){o.inBitmap=null;return BitmapFactory.decodeByteArray(data,0,data.length,o);}}
    ...
    private File privateDir(){return new File(context.getFilesDir(),"yoru-image-cache");}
    private void ensurePrivateDir(){try{File dir=privateDir();if(!dir.exists())dir.mkdirs();new File(dir,".nomedia").createNewFile();}catch(Exception ignored){}}
    private File privateFile(String key){String hash=Integer.toHexString(key.hashCode());try{byte[] bytes=MessageDigest.getInstance("SHA-256").digest(key.getBytes(StandardCharsets.UTF_8));StringBuilder out=new StringBuilder();for(byte b:bytes)out.append(String.format(Locale.ROOT,"%02x",b&255));hash=out.toString();}catch(Exception ignored){}return new File(privateDir(),hash+".bin");}
    private void saveRaw(File file,byte[] raw){try{ensurePrivateDir();File tmp=new File(file.getParentFile(),file.getName()+".tmp");try(OutputStream out=new BufferedOutputStream(new FileOutputStream(tmp))){out.write(raw);}if(file.exists())file.delete();tmp.renameTo(file);}catch(Exception ignored){}}
    ...
    private void trimMaybe(){long now=System.currentTimeMillis();if(now-lastTrim<60*60*1000L)return;lastTrim=now;File[] files=privateDir().listFiles();if(files==null||files.length<360)return;long size=0;for(File f:files)size+=f.length();if(size<160L*1024*1024)return;Arrays.sort(files,Comparator.comparingLong(File::lastModified));for(File f:files){if(f.getName().equals(".nomedia"))continue;long n=f.length();if(f.delete())size-=n;if(size<120L*1024*1024)break;}}
    private void deleteOldDiskCache(){erase(new File(context.getCacheDir(),"covers"));}
