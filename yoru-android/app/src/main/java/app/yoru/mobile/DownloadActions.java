...
public final class DownloadActions {
    ...
    private static void showBatch(Activity activity,Anime anime,Anime.Playback ready,ArrayList<Anime.Episode> episodes,double suggested){ArrayList<Anime.Episode> rows=EpisodeRules.downloadable(anime,episodes);if(rows.isEmpty()){Ui.toast(activity,"Нет вышедших серий для скачивания");return;}String[] presets={"Следующие 3 серии","Следующие 6 серий","Следующие 12 серий","Все вышедшие серии","Выбрать серии вручную"};Ui.choices(activity,"Массовое скачивание",presets,which->{if(which==4)multiBatch(activity,anime,ready,rows,suggested);else{int count=which==0?3:which==1?6:which==2?12:Integer.MAX_VALUE;int start=closest(rows,suggested);ArrayList<Anime.Episode> selected=new ArrayList<>();for(int i=start;i<rows.size()&&selected.size()<count;i++)selected.add(rows.get(i));qualityBatch(activity,anime,ready,selected);}});}
