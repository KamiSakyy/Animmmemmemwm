...
final class NativeFrame {
    private static final java.util.concurrent.atomic.AtomicBoolean capturing=new java.util.concurrent.atomic.AtomicBoolean();
    private NativeFrame(){}
    static void capture(Activity activity,PlayerView playerView){
        ...
        Bitmap bitmap;
        try{bitmap=Bitmap.createBitmap(Math.max(1,(int)(width*scale)),Math.max(1,(int)(height*scale)),Bitmap.Config.ARGB_8888);}catch(OutOfMemoryError e){capturing.set(false);Ui.toast(activity,"Недостаточно памяти для кадра");return;}
        try{PixelCopy.request((SurfaceView)surface,bitmap,result->{
            if(result!=PixelCopy.SUCCESS){capturing.set(false);bitmap.recycle();if(!activity.isFinishing())Ui.toast(activity,"Кадр недоступен или защищён");return;}
            java.util.concurrent.Future<?> save=YoruApp.app().io.submit(()->{Uri uri=null;File file=null;boolean gallery=Build.VERSION.SDK_INT>=29;try{
                ...
                Uri saved=uri;activity.runOnUiThread(()->{if(activity.isFinishing())return;if(gallery)Ui.toast(activity,"Кадр сохранён в галерею");else{Intent share=new Intent(Intent.ACTION_SEND).setType("image/png").putExtra(Intent.EXTRA_STREAM,saved).addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);share.setClipData(ClipData.newRawUri("YORU",saved));try{activity.startActivity(Intent.createChooser(share,"Сохранить или отправить кадр"));}catch(Exception e){Ui.toast(activity,"Кадр сохранён в приложении");}}});
            }catch(Exception e){if(gallery&&uri!=null)try{activity.getContentResolver().delete(uri,null,null);}catch(Exception ignored){}if(file!=null)file.delete();activity.runOnUiThread(()->{if(!activity.isFinishing())Ui.toast(activity,"Не удалось сохранить кадр");});}finally{bitmap.recycle();capturing.set(false);}});if(save.isCancelled()){bitmap.recycle();capturing.set(false);}
        },new Handler(Looper.getMainLooper()));}catch(IllegalArgumentException e){capturing.set(false);bitmap.recycle();Ui.toast(activity,"Видеокадр пока недоступен");}
    }
}
