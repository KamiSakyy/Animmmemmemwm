...
final class QualityPlus {
  ...
  static final String[] LABELS_WITH_BEST = {
    "360p",
    "480p",
    "720p",
    "1080p",
  ...
  static int index(int value) {
    int q = clamp(value);
    for (int i = 0; i < VALUES.length; i++) if (VALUES[i] == q) return i;
    return 2;
  }
  ...
  static String name(int value) {
    ...
    if (q <= 1440) return "1440p Quality+";
    return "2160p Quality+";
  }
  ...
  static int bestAtOrBelow(Collection<Integer> rows, int cap) {
    ...
    int limit = cap >= BEST ? Integer.MAX_VALUE : clamp(cap),
      best = 0;
    for (Integer q : rows) {
      if (q == null) continue;
      int v = q;
      ...
      if (v <= limit && v > best) best = v;
    }
    if (best == 0) for (Integer q : rows)
      if (q != null) {
        best = q;
        break;
      }
