...
final class NativePlayers {
    ...
    static void limitQuality(ExoPlayer player,int height){
        player.setTrackSelectionParameters(player.getTrackSelectionParameters().buildUpon().setMaxVideoSize(Integer.MAX_VALUE,height>0?height:Integer.MAX_VALUE).setForceLowestBitrate(false).build());
