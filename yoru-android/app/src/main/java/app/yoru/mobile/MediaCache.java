...
public final class MediaCache {
  ...
  private SimpleCache stream, offline;

  public MediaCache(Context c) {
  ...
  public synchronized SimpleCache offline() {
    if (offline == null) offline = new SimpleCache(
      new File(context.getFilesDir(), "offline-media"),
  ...
  public synchronized SimpleCache temporary() {
    if (stream == null) stream = new SimpleCache(
  ...
  public CacheDataSource.Factory offlineFactory() {
    return new CacheDataSource.Factory()
      .setCache(offline())
  ...
  public synchronized long offlineBytes() {
    return offline().getCacheSpace();
  }

  public synchronized void clearVideoCache() {
    if (stream != null) {
      stream.release();
      stream = null;
    }
    erase(new File(context.getCacheDir(), "video"));
  }
  ...
  public static long size(File f) {
    ...
    if (children != null) for (File x : children) n += size(x);
    return n;
  }

  public static void erase(File f) {
    if (!f.exists()) return;
    if (f.isDirectory()) {
      File[] children = f.listFiles();
      if (children != null) for (File x : children) erase(x);
    }
    f.delete();
  }

  public long temporaryBytes() {
    long n = size(context.getCacheDir());
