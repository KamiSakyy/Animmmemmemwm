...
final class InlinePlayer extends LinearLayout {
    ...
    void offerEpisodes(Anime owner,Anime.Playback value){
        ...
        playback=value;episodeButton.setText("Список серий");
    }
    private void prepare(){prepare(false);}
    private void prepare(boolean selectEpisode){
        ...
        tasks.submit(YoruApp.app().ui,()->{try{Anime.Playback result=YoruApp.app().api.playback(selected,"yoru");TaskQueue.check();if(result==null||result.video==null)throw new java.io.IOException();post(()->{if(!alive(token))return;playback=result;JSONObject history=YoruApp.app().store.progress(anime);double number=history.optDouble("episode",1);Anime.Episode found=null;for(Anime.Episode row:EpisodeRules.visible(anime,result.video.episodeList))if(!row.future&&(found==null||Math.abs(row.number-number)<.001)){found=row;if(Math.abs(row.number-number)<.001)break;}state(false,"Серии загружены");if(selectEpisode){chooseEpisode();return;}if(found!=null)load(found);else status.setText("Доступных серий пока нет");});}catch(Exception e){post(()->{if(alive(token))state(false,"Не удалось получить серии. Нажмите «Смотреть», чтобы повторить.");});}},()->state(false,"Очередь занята. Повторите позже."));
    }
    private void load(Anime.Episode value){
        if(value==null||value.future)return;playRequested=true;resumePosition=0;release();tasks.cancel();int token=++generation;episode=value;voice="";streams.clear();state(true,"Готовим серию "+Ui.number(value.number)+"…");episodeButton.setText("Серия "+Ui.number(value.number));
        tasks.submit(YoruApp.app().ui,()->{try{YoruApp.app().api.loadEpisode(value);TaskQueue.check();post(()->{if(!alive(token))return;state(false,"Выберите озвучку");ArrayList<Anime.Variant> variants=variants();SecureStore store=YoruApp.app().store;boolean preferred=store.onlyPreferredVoice()||!store.voicePreference().isEmpty()||!store.favoriteVoices().isEmpty();boolean matches=!store.voicePreference().isEmpty()&&ApiRepository.voiceMatches(store.voicePreference(),ApiRepository.sourceVoice(playback.video.source));if(!value.streams.isEmpty()&&(matches||((!preferred||variants.isEmpty())&&!store.onlyPreferredVoice()))){voice=ApiRepository.sourceVoice(playback.video.source);accept(value.streams);}else if(!variants.isEmpty())resolve(variants.get(0));else status.setText("Выбранная озвучка недоступна для этой серии");});}catch(Exception e){post(()->{if(alive(token))state(false,"Серия недоступна. Выберите другую серию.");});}},()->state(false,"Очередь занята. Повторите позже."));
    }
    ...
    private void play(String url,long position){
        ...
        tasks.submit(YoruApp.app().ui,()->{try{long bytes=MediaSize.probe(url);TaskQueue.check();post(()->{if(!alive(token))return;sizeLabel=MediaSize.label(bytes);startVideo(url,position);});}catch(Exception error){post(()->{if(alive(token))state(false,"Не удалось подготовить видео. Повторите позже.");});}},()->state(false,"Очередь занята. Повторите позже."));
    }
    private void startVideo(String url,long position){
        ...
        JSONObject history=YoruApp.app().store.progress(anime);long seek=position>=0?position:Math.abs(history.optDouble("episode",-1)-episode.number)<.001?history.optLong("time",0)*1000L:0;player.seekTo(Math.max(0,seek));player.setPlaybackSpeed(YoruApp.app().store.playbackSpeed());player.setPlayWhenReady(playRequested);player.prepare();qualityButton.setText("Разрешение · "+QualityPlus.streamLabel(quality));state(false,"Серия "+Ui.number(episode.number)+" · "+QualityPlus.streamLabel(quality)+" · "+sizeLabel);removeCallbacks(checkpoint);postDelayed(checkpoint,5000);
    }
    private void chooseEpisode(){if(playback==null){prepare(true);return;}ArrayList<Anime.Episode> rows=new ArrayList<>();for(Anime.Episode row:EpisodeRules.visible(anime,playback.video.episodeList))if(!row.future)rows.add(row);if(rows.isEmpty()){Ui.toast(activity,"Доступных серий пока нет");return;}String[] labels=new String[rows.size()];for(int i=0;i<labels.length;i++)labels[i]=rows.get(i).label();Ui.choices(activity,"Список серий",labels,index->load(rows.get(index)));}
    private void chooseVoice(){ArrayList<Anime.Variant> rows=episode==null?new ArrayList<>():new ArrayList<>(SourceEngine.sortVariants(episode.variants));if(rows.isEmpty()){Ui.toast(activity,"Сначала откройте доступную серию");return;}String[] labels=new String[rows.size()];for(int i=0;i<labels.length;i++)labels[i]=rows.get(i).label();Ui.choices(activity,"Озвучка",labels,index->{Anime.Variant selected=rows.get(index);String name=ApiRepository.voiceTitle(selected.name.isEmpty()?selected.label():selected.name);if(!name.isEmpty()){YoruApp.app().store.voicePreference(name);YoruApp.app().store.rememberVoice(name);}resolve(selected);});}
    ...
    private void chooseSeason(){if(anime==null||busy)return;int token=generation;state(true,"Загружаем части истории…");Anime selected=Anime.from(anime.json());tasks.submit(YoruApp.app().ui,()->{try{List<Anime> rows=YoruApp.app().api.franchise(selected);post(()->{if(!alive(token))return;state(false,"Выберите сезон или часть истории");seasons.clear();seasons.addAll(rows);String[] labels=new String[seasons.size()];for(int i=0;i<labels.length;i++){Anime row=seasons.get(i);labels[i]=row.title+(row.year>0?" · "+row.year:"");}Ui.choices(activity,"Сезон / часть",labels,index->{suspend();anime=seasons.get(index);playback=null;episode=null;streams.clear();seasonButton.setText(anime.title);episodeButton.setText("Список серий");resumePosition=0;playRequested=false;if(seasonListener!=null)seasonListener.accept(anime);else prepare(true);});});}catch(Exception e){post(()->{if(alive(token))state(false,"Список частей сейчас недоступен");});}},()->state(false,"Очередь занята. Повторите позже."));}
    ...
    void suspend(){generation++;tasks.cancel();if(player!=null){resumePosition=Math.max(0,player.getCurrentPosition());playRequested=player.getPlayWhenReady();}release();busy=false;if(!closed)state(false,"Просмотр приостановлен");}
