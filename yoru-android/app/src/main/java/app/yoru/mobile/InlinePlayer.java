...
final class InlinePlayer extends LinearLayout {
    ...
    private final TextView status,start,episodeButton,voiceButton,qualityButton,seasonButton,lockButton;
    ...
    private boolean busy,closed,locked,playRequested=true,activeSession,resumePending;
    private long resumePosition=-1;private double resumeEpisode=-1;
    ...
    InlinePlayer(Activity activity){
        ...
        start=Ui.button(activity,"Смотреть",true,()->{if(player!=null){if(player.getPlayWhenReady())player.pause();else{if(player.getPlaybackState()==Player.STATE_IDLE)player.prepare();player.play();}}else{playRequested=true;prepare();}});Ui.space(this,10);addView(start,Ui.lp(activity,-1,-2));
        ...
        tools.addView(Ui.iconButton(activity,"expand","На весь экран",()->{if(anime==null||locked)return;double number=episode==null?1:episode.number;suspend();activeSession=false;resumePending=false;activity.startActivity(new android.content.Intent(activity,PlayerActivity.class).putExtra("anime",anime.json().toString()).putExtra("mode","yoru").putExtra("episode",number).putExtra("voice",YoruApp.app().store.voicePreference().isEmpty()?voice:YoruApp.app().store.voicePreference()).putExtra("quality",quality>0?quality:YoruApp.app().store.quality()).putExtra("fullscreen",true).putExtra("playRequested",playRequested).putExtra("position",resumePosition));}),Ui.lp(activity,48,48));
        lockButton=Ui.button(activity,"Блокировка",false,()->setLocked(!locked));tools.addView(lockButton,new LayoutParams(0,-2,1));addView(tools);
    }
    private void setLocked(boolean value){locked=value;video.setUseController(!locked);state(busy,status.getText().toString());lockButton.setText(locked?"Разблокировать":"Блокировка");}
    android.os.Bundle snapshot(){android.os.Bundle state=new android.os.Bundle();if(anime!=null)state.putString("anime",anime.json().toString());state.putInt("quality",requestedQuality);state.putBoolean("locked",locked);state.putBoolean("active",activeSession);state.putBoolean("play",player==null?playRequested:player.getPlayWhenReady());state.putLong("position",player==null?resumePosition:player.getCurrentPosition());state.putDouble("episode",episode==null?resumeEpisode:episode.number);return state;}
    void restore(android.os.Bundle state){if(state==null)return;try{Anime saved=Anime.from(new JSONObject(state.getString("anime","")));if(Anime.valid(saved))anime=saved;}catch(Exception ignored){}requestedQuality=state.getInt("quality",requestedQuality);if(requestedQuality<0||requestedQuality>4320)requestedQuality=YoruApp.app().store.quality();quality=requestedQuality;resumePosition=Math.max(-1,state.getLong("position",-1));resumeEpisode=state.getDouble("episode",-1);if(!Double.isFinite(resumeEpisode)||resumeEpisode<0)resumeEpisode=-1;playRequested=state.getBoolean("play",true);activeSession=state.getBoolean("active",false);resumePending=activeSession;setLocked(state.getBoolean("locked",false));if(anime!=null)seasonButton.setText(anime.title);}
    void resume(){if(!resumePending||!activeSession||closed||busy)return;resumePending=false;if(playback!=null&&episode!=null)load(episode);else prepare();}
    ...
    private void prepare(){
        ...
        activeSession=true;resumePending=false;int token=++generation;tasks.cancel();state(true,"Загружаем серии…");Anime selected=Anime.from(anime.json());
        tasks.submit(YoruApp.app().ui,()->{try{Anime.Playback result=YoruApp.app().api.playback(selected,"yoru");TaskQueue.check();post(()->{if(!alive(token))return;playback=result;JSONObject history=YoruApp.app().store.progress(anime);double number=resumeEpisode>=0?resumeEpisode:history.optDouble("episode",1);Anime.Episode found=null;for(Anime.Episode row:EpisodeRules.visible(anime,result.video.episodeList))if(!row.future&&(found==null||Math.abs(row.number-number)<.001)){found=row;if(Math.abs(row.number-number)<.001)break;}state(false,"Серии загружены");if(found!=null)load(found);else status.setText("Доступных серий пока нет");});}catch(Exception e){post(()->{if(alive(token))state(false,"Не удалось получить серии. Нажмите «Смотреть», чтобы повторить.");});}},()->state(false,"Очередь занята. Повторите позже."));
    }
    private void load(Anime.Episode value){
        if(value==null||value.future)return;release();if(Double.compare(resumeEpisode,value.number)!=0){resumePosition=-1;playRequested=true;}resumeEpisode=value.number;activeSession=true;resumePending=false;tasks.cancel();int token=++generation;episode=value;voice="";streams.clear();state(true,"Готовим серию "+Ui.number(value.number)+"…");episodeButton.setText("Серия "+Ui.number(value.number));
    ...
    private void startNative(String url,long position,long bytes){
        ...
        player.addListener(new Player.Listener(){@Override public void onPlayWhenReadyChanged(boolean ready,int reason){playRequested=ready;start.setText(ready?"Пауза":"Продолжить");}@Override public void onIsPlayingChanged(boolean playing){start.setText(player!=null&&player.getPlayWhenReady()?"Пауза":"Продолжить");}@Override public void onPlayerError(PlaybackException error){state(false,"Видео не открылось. Выберите другую озвучку или разрешение.");}@Override public void onPlaybackStateChanged(int state){if(state==Player.STATE_ENDED&&YoruApp.app().autoNextAllowed()&&playback!=null){Anime.Episode next=null;for(Anime.Episode row:EpisodeRules.visible(anime,playback.video.episodeList))if(!row.future&&episode!=null&&row.number>episode.number&&(next==null||row.number<next.number))next=row;if(next!=null)load(next);}}});
        player.setTrackSelectionParameters(player.getTrackSelectionParameters().buildUpon().setMaxVideoSize(Integer.MAX_VALUE,quality>0?quality:Integer.MAX_VALUE).setForceLowestBitrate(false).build());player.setMediaSource(NativePlayers.source(url));
        JSONObject history=YoruApp.app().store.progress(anime);long seek=position>=0?position:resumePosition>=0?resumePosition:Math.abs(history.optDouble("episode",-1)-episode.number)<.001?history.optLong("time",0)*1000L:0;player.seekTo(Math.max(0,seek));resumePosition=-1;player.setPlaybackSpeed(YoruApp.app().store.playbackSpeed());qualityButton.setText("Разрешение · "+QualityPlus.streamLabel(quality));state(false,"Серия "+Ui.number(episode.number)+" · "+QualityPlus.streamLabel(quality)+" · "+MediaSize.label(bytes));player.setPlayWhenReady(playRequested);player.prepare();start.setText(playRequested?"Пауза":"Продолжить");removeCallbacks(checkpoint);postDelayed(checkpoint,5000);
    }
    ...
    private void chooseSeason(){if(anime==null||busy)return;int token=generation;state(true,"Загружаем части истории…");Anime selected=Anime.from(anime.json());tasks.submit(YoruApp.app().ui,()->{try{List<Anime> rows=YoruApp.app().api.franchise(selected);post(()->{if(!alive(token))return;state(false,"Выберите сезон или часть истории");seasons.clear();seasons.addAll(rows);String[] labels=new String[seasons.size()];for(int i=0;i<labels.length;i++){Anime row=seasons.get(i);labels[i]=row.title+(row.year>0?" · "+row.year:"");}Ui.choices(activity,"Сезон / часть",labels,index->{suspend();anime=seasons.get(index);playback=null;episode=null;resumeEpisode=-1;resumePosition=-1;playRequested=true;streams.clear();seasonButton.setText(anime.title);episodeButton.setText("Серия");prepare();});});}catch(Exception e){post(()->{if(alive(token))state(false,"Список частей сейчас недоступен");});}},()->state(false,"Очередь занята. Повторите позже."));}
    ...
    private void release(){removeCallbacks(checkpoint);if(player!=null){save();boolean wanted=player.getPlayWhenReady();resumePosition=player.getCurrentPosition();if(episode!=null)resumeEpisode=episode.number;video.setPlayer(null);player.release();player=null;playRequested=wanted;YoruApp.app().activePlayers=Math.max(0,YoruApp.app().activePlayers-1);}}
    void suspend(){generation++;tasks.cancel();release();resumePending=activeSession;busy=false;if(!closed)state(false,"Просмотр приостановлен");}
