...
final class InlinePlayer extends LinearLayout {
    ...
    private void save(){if(player==null||anime==null||episode==null||(player.getPlaybackState()!=Player.STATE_READY&&player.getPlaybackState()!=Player.STATE_ENDED))return;long duration=player.getDuration();YoruApp.app().store.progress(anime,episode.number,(int)(player.getCurrentPosition()/1000),duration==C.TIME_UNSET?0:(int)Math.max(0,duration/1000),"yoru",voice,true);}
