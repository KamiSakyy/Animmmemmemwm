...
final class InlinePlayer extends LinearLayout {
    ...
    private int generation,quality;
    ...
    InlinePlayer(Activity activity){
        super(activity);this.activity=activity;setOrientation(VERTICAL);setPadding(Ui.dp(activity,12),Ui.dp(activity,12),Ui.dp(activity,12),Ui.dp(activity,12));setBackground(Ui.stroke(Ui.CARD,16,activity));
        ...
        tools.addView(Ui.iconButton(activity,"camera","Снимок видеокадра",()->NativeFrame.capture(activity,video)),Ui.lp(activity,48,48));
        tools.addView(Ui.iconButton(activity,"expand","На весь экран",()->{if(anime==null)return;double number=episode==null?1:episode.number;suspend();Ui.openPlayer(activity,anime,"yoru",number);}),Ui.lp(activity,48,48));
    ...
    private void prepare(){
        ...
        tasks.submit(YoruApp.app().ui,()->{try{Anime.Playback result=YoruApp.app().api.playback(selected,"yoru");TaskQueue.check();post(()->{if(!alive(token))return;playback=result;JSONObject history=YoruApp.app().store.progress(anime);double number=history.optDouble("episode",1);Anime.Episode found=null;for(Anime.Episode row:result.video.episodeList)if(!row.future&&(found==null||Math.abs(row.number-number)<.001)){found=row;if(Math.abs(row.number-number)<.001)break;}state(false,"Серии загружены");if(found!=null)load(found);else status.setText("Доступных серий пока нет");});}catch(Exception e){post(()->{if(alive(token))state(false,"Не удалось получить серии. Нажмите «Смотреть», чтобы повторить.");});}},()->state(false,"Очередь занята. Повторите позже."));
    }
    ...
    private void accept(Map<Integer,String> values){streams.clear();for(Map.Entry<Integer,String> row:values.entrySet())if(!ApiRepository.safeUrl(row.getValue()).isEmpty())streams.put(row.getKey(),row.getValue());if(streams.isEmpty()){state(false,"Нет доступного видео для собственного плеера");return;}quality=QualityPlus.bestAtOrBelow(streams.keySet(),YoruApp.app().store.quality());voiceButton.setText(voice.isEmpty()?"Озвучка источника":voice);play(streams.get(quality),-1);}
    private void play(String url,long position){
        if(url==null||url.isEmpty())return;release();player=new ExoPlayer.Builder(activity).build();YoruApp.app().activePlayers++;player.setAudioAttributes(new AudioAttributes.Builder().setUsage(C.USAGE_MEDIA).setContentType(C.AUDIO_CONTENT_TYPE_MOVIE).build(),true);player.setHandleAudioBecomingNoisy(true);video.setPlayer(player);
        player.addListener(new Player.Listener(){@Override public void onIsPlayingChanged(boolean playing){start.setText(playing?"Пауза":"Продолжить");}@Override public void onPlayerError(PlaybackException error){state(false,"Видео не открылось. Выберите другую озвучку или разрешение.");}@Override public void onPlaybackStateChanged(int state){if(state==Player.STATE_ENDED&&YoruApp.app().autoNextAllowed()&&playback!=null){for(Anime.Episode row:playback.video.episodeList)if(!row.future&&episode!=null&&row.number>episode.number){load(row);break;}}}});
        player.setMediaSource(new DefaultMediaSourceFactory(YoruApp.app().mediaCache.onlineFactory()).createMediaSource(DownloadHub.item(url)));
        JSONObject history=YoruApp.app().store.progress(anime);long seek=position>=0?position:Math.abs(history.optDouble("episode",-1)-episode.number)<.001?history.optLong("time",0)*1000L:0;player.seekTo(Math.max(0,seek));player.setPlaybackSpeed(YoruApp.app().store.playbackSpeed());player.prepare();player.play();qualityButton.setText("Разрешение · "+QualityPlus.streamLabel(quality));state(false,"Серия "+Ui.number(episode.number)+" · "+QualityPlus.streamLabel(quality));int token=generation;tasks.submit(YoruApp.app().discovery,()->{long size=MediaSize.probe(url);post(()->{if(alive(token)&&url.equals(streams.get(quality)))status.setText("Серия "+Ui.number(episode.number)+" · "+QualityPlus.streamLabel(quality)+" · "+MediaSize.label(size));});},null);removeCallbacks(checkpoint);postDelayed(checkpoint,5000);
    }
    private void chooseEpisode(){if(playback==null){prepare();return;}ArrayList<Anime.Episode> rows=new ArrayList<>();for(Anime.Episode row:playback.video.episodeList)if(!row.future)rows.add(row);String[] labels=new String[rows.size()];for(int i=0;i<labels.length;i++)labels[i]=rows.get(i).label();Ui.choices(activity,"Серия",labels,index->load(rows.get(index)));}
    private void chooseVoice(){ArrayList<Anime.Variant> rows=variants();if(rows.isEmpty()){Ui.toast(activity,"Сначала откройте доступную серию");return;}String[] labels=new String[rows.size()];for(int i=0;i<labels.length;i++)labels[i]=rows.get(i).label();Ui.choices(activity,"Озвучка",labels,index->resolve(rows.get(index)));}
    private void chooseQuality(){ArrayList<Integer> rows=new ArrayList<>(streams.keySet());if(rows.isEmpty())return;String[] labels=new String[rows.size()];for(int i=0;i<labels.length;i++)labels[i]=QualityPlus.streamLabel(rows.get(i));Ui.choices(activity,"Разрешение",labels,index->{long position=player==null?-1:player.getCurrentPosition();quality=rows.get(index);play(streams.get(quality),position);});}
