...
final class InlinePlayer extends LinearLayout {
    ...
    private void load(Anime.Episode value,boolean autoplay,long position){
        if(value==null||value.future)return;restorePending=false;playRequested=autoplay;initialPosition=position;resumePosition=Math.max(0,position);release();tasks.cancel();int token=++generation;episode=value;voice="";streams.clear();state(true,"Готовим серию "+Ui.number(value.number)+"…");episodeButton.setText("Серия "+Ui.number(value.number));
