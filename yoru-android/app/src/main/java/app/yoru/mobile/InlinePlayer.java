...
final class InlinePlayer extends LinearLayout {
    ...
    InlinePlayer(Activity activity){
        ...
        start=Ui.button(activity,"Смотреть",true,()->{if(player!=null){if(player.getPlayWhenReady())player.pause();else{if(player.getPlaybackState()==Player.STATE_IDLE)player.prepare();player.play();}}else prepare();});Ui.space(this,10);addView(start,Ui.lp(activity,-1,-2));
