...
public final class DetailsActivity extends Activity {
  ...
  private int generation, downloadsGeneration;
  private boolean loaded, episodesLoaded, favoriteCached;
  private String bucketCached = "",
    relatedFilter = "";
  private JSONObject progressCached = new JSONObject();
  private HashMap<
    String,
  ...
  public void onCreate(Bundle b) {
    ...
    TextView label = Ui.text(this, "YORU", 13, Ui.MUTED, true);
  ...
  private void chips() {
    if (!anime.original.isEmpty() && !anime.original.equals(anime.title)) {
      body.addView(Ui.text(this, anime.original, 12, Ui.MUTED, false));
      Ui.space(body, 13);
    }
    if (!anime.genres.isEmpty()) {
      ...
      for (String g : anime.genres.subList(
        0,
        Math.min(10, anime.genres.size())
      )) {
        TextView chip = Ui.chip(this, g, false, null);
        LinearLayout.LayoutParams p = Ui.lp(this, -2, -2);
        p.rightMargin = Ui.dp(this, 7);
        row.addView(chip, p);
      }
      h.addView(row);
      body.addView(h, Ui.lp(this, -1, -2));
      Ui.space(body, 14);
    }
    String status = anime.statusLabel();
    String eps = anime.episodesLine();
    String next =
      anime.ongoing() && !anime.nextEpisodeAt.isEmpty()
        ? " · следующая: " + airText(anime.nextEpisodeAt)
        : "";
    body.addView(
      Ui.text(
        this,
  ...
  private void actions() {
    JSONObject history = progressCached;
    String play = history.length() > 0 ? "Продолжить" : "Смотреть";
    String storedMode = history.optString("playerMode", "");
    if (storedMode.trim().isEmpty() || storedMode.equals("auto")) storedMode =
      "yoru";
    final String playMode = storedMode;
    double startEpisode = history.optDouble("episode", 1);
    androidx.media3.exoplayer.offline.Download offline = completed.get(
      DownloadHub.episodeKey(startEpisode)
    );
    TextView start = Ui.button(
      this,
      anime.blocked
        ? "Просмотр ограничен"
        : offline != null
      ...
      true,
      () -> {
        if (offline != null) Ui.openOffline(
          this,
          offline.request.id,
          anime,
          startEpisode
        );
        else openWatch(startEpisode, playMode);
      }
    );
    start.setEnabled(!anime.blocked);
    if (anime.blocked) start.setAlpha(.45f);
    body.addView(start, Ui.lp(this, -1, -2));
    Ui.space(body, 10);
    voiceInfoCard();
    Ui.space(body, 10);
    LinearLayout main = Ui.row(this);
    main.addView(
      Ui.button(
        this,
        favoriteCached ? "В коллекции" : "В коллекцию",
        false,
        () -> Ui.bucketDialog(this, anime, this::render)
      ),
      new LinearLayout.LayoutParams(0, -2, 1)
    );
    LinearLayout.LayoutParams dp = new LinearLayout.LayoutParams(0, -2, 1);
    dp.leftMargin = Ui.dp(this, 9);
    main.addView(
      Ui.button(this, "Скачать · " + downloadVoiceName(), false, () ->
        DownloadActions.chooseEpisodeFast(
          this,
          anime,
          null,
          history.optDouble("episode", 1),
          anime.episodeList
        )
      ),
      dp
  ...
  private void progress() {
    ...
    if (history.length() > 0) {
      ...
      body.addView(
        Ui.text(
          this,
          "Вы смотрели серию " +
            Ui.number(history.optDouble("episode", 1)) +
            " · " +
            Ui.time(history.optInt("time")),
          11,
          Ui.MUTED,
          false
        )
    ...
    String bucket = bucketCached;
    if (!bucket.isEmpty()) {
      int i = Arrays.asList(SecureStore.BUCKETS).indexOf(bucket);
      Ui.space(body, 7);
      body.addView(
        Ui.text(
          this,
          "Ваш список: " +
            (i >= 0 ? SecureStore.BUCKET_LABELS[i] : "В планах") +
            (YoruApp.app().store.folderSummary(anime).isEmpty()
              ? ""
              : " · папки: " + YoruApp.app().store.folderSummary(anime)),
          11,
          Ui.PURPLE,
          false
        )
    ...
    String hint = anime.episodeList.isEmpty()
      ? episodesLoaded
        ? "Список серий пока недоступен. Попробуйте открыть просмотр."
        : "Карточка открыта. Серии появятся после ответа каталога."
      : "Просмотр: " +
        defaultVoiceName() +
        " · " +
        qualityName(YoruApp.app().store.quality()) +
        ".";
    state = Ui.text(this, hint, 11, Ui.MUTED, false);
  ...
  private void about() {
    Ui.space(body, 24);
    body.addView(Ui.text(this, "Об истории", 21, Ui.TEXT, true));
    Ui.space(body, 13);
    if (YoruApp.app().store.spoilerSafe() && !anime.description.isEmpty()) {
      LinearLayout c = card();
      c.addView(
        Ui.text(
          this,
          "Описание скрыто режимом без спойлеров.",
          12,
          Ui.MUTED,
          false
        )
      ...
      Ui.space(c, 9);
      c.addView(
        Ui.button(this, "Показать описание", false, () -> {
          YoruApp.app().store.smartSetting("spoilerSafe", false);
          render();
        })
      );
      body.addView(c);
    ...
    description.setLineSpacing(Ui.dp(this, 5), 1);
    body.addView(description);
  }

  private void trailer() {
    try {
      String url = ApiRepository.safeUrl(anime.trailerUrl);
      if (url.isEmpty()) url =
        "https://www.youtube.com/results?search_query=" +
        Uri.encode(YoruBrain.title(anime) + " trailer");
      startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse(url)));
    } catch (Exception e) {
      Ui.toast(this, "Не удалось открыть трейлер");
    }
  }

  private void similar() {
    ArrayList<Anime> rows = YoruBrain.similar(anime, 8);
    if (rows.isEmpty()) return;
    Ui.space(body, 26);
    body.addView(Ui.text(this, "Похожее", 21, Ui.TEXT, true));
    Ui.space(body, 12);
    ...
    row.addView(list);
    body.addView(row, Ui.lp(this, -1, -2));
  }

  private void related() {
    if (anime.related.isEmpty()) return;
    ArrayList<Anime> rows = filteredRelated();
    Ui.space(body, 27);
    LinearLayout title = Ui.row(this);
    title.addView(
      Ui.text(this, "Связанные аниме и фильмы", 21, Ui.TEXT, true),
      new LinearLayout.LayoutParams(0, -2, 1)
    );
    Anime next = nextFranchise();
    if (next != null) {
      TextView cont = Ui.text(
        this,
        franchiseEnded() ? "Продолжить" : "Дальше",
        10,
        Ui.PURPLE,
        true
      );
      ...
      cont.setBackground(Ui.stroke(Ui.SURFACE, 10, this));
      Ui.press(cont);
      cont.setOnClickListener(v -> Ui.openDetails(this, next));
      title.addView(cont);
    }
    body.addView(title);
    Ui.space(body, 10);
    body.addView(
      Ui.button(this, "Смотреть по порядку франшизу", false, () -> {
        ArrayList<Anime> seq = franchiseSequence();
        if (!seq.isEmpty()) Ui.openDetails(this, seq.get(0));
      }),
      Ui.lp(this, -1, -2)
    );
    Ui.space(body, 10);
    ...
    for (String[] f : fs) {
      TextView chip = Ui.chip(this, f[1], f[0].equals(relatedFilter), () -> {
        relatedFilter = f[0];
        render();
      });
    ...
    wrap.addView(chips);
    body.addView(wrap, Ui.lp(this, -1, -2));
    Ui.space(body, 12);
    if (rows.isEmpty()) {
      LinearLayout c = card();
      c.addView(
        Ui.text(this, "В этом фильтре пока пусто.", 12, Ui.MUTED, false)
      );
      body.addView(c);
      return;
    }
    for (Anime a : rows) {
      ...
      row.setBackground(Ui.stroke(Ui.CARD, 15, this));
      ImageView poster = new ImageView(this);
      poster.setScaleType(ImageView.ScaleType.CENTER_CROP);
      poster.setClipToOutline(true);
      poster.setBackground(Ui.shape(Ui.SURFACE, 12, this));
      row.addView(poster, Ui.lp(this, 48, 68));
      YoruApp.app().images.load(poster, a);
      String when =
        a.airedDate == null || a.airedDate.isEmpty()
          ? a.meta()
          : a.airedDate + " · " + a.type;
      TextView text = Ui.text(this, a.title + "\n" + when, 12, Ui.TEXT, false);
      text.setPadding(Ui.dp(this, 12), 0, 0, 0);
      row.addView(text, new LinearLayout.LayoutParams(0, -2, 1));
      Ui.Icon icon = new Ui.Icon(this, "play");
      icon.color = Ui.PURPLE;
      row.addView(icon, Ui.lp(this, 24, 24));
      Ui.press(row);
      row.setOnClickListener(v -> Ui.openDetails(this, a));
      body.addView(row);
      Ui.space(body, 9);
    }
  }

  private ArrayList<Anime> filteredRelated() {
    ArrayList<Anime> out = new ArrayList<>();
    for (Anime a : anime.related) {
      String t = (a.type == null ? "" : a.type).toLowerCase(Locale.ROOT);
      boolean film = t.contains("фильм") || t.contains("movie");
      boolean ova = t.contains("ova");
      boolean special = t.contains("спеш") || t.contains("special");
      boolean season = !film && !ova && !special;
      if (
        relatedFilter.isEmpty() ||
        ("film".equals(relatedFilter) && film) ||
        ("ova".equals(relatedFilter) && ova) ||
        ("special".equals(relatedFilter) && special) ||
        ("season".equals(relatedFilter) && season)
      ) out.add(a);
    }
    out.sort(this::franchiseCompare);
    return out;
  }

  private ArrayList<Anime> franchiseSequence() {
    LinkedHashMap<String, Anime> map = new LinkedHashMap<>();
    if (Anime.valid(anime)) map.put(SourceEngine.identity(anime), anime);
    for (Anime a : anime.related)
      if (Anime.valid(a)) map.put(SourceEngine.identity(a), a);
    ArrayList<Anime> rows = new ArrayList<>(map.values());
    rows.sort(this::franchiseCompare);
    return rows;
  }

  private Anime nextFranchise() {
    ArrayList<Anime> rows = franchiseSequence();
    String own = SourceEngine.identity(anime);
    for (int i = 0; i < rows.size(); i++) if (
      SourceEngine.identity(rows.get(i)).equals(own) && i + 1 < rows.size()
    ) return rows.get(i + 1);
    return null;
  }

  private boolean franchiseEnded() {
    double ep = progressCached.optDouble("episode", 0);
    int end = anime.episodesAired > 0 ? anime.episodesAired : anime.episodes;
    return end > 0 && ep >= end;
  }

  private int franchiseCompare(Anime a, Anime b) {
    int p = releaseOrder(a) - releaseOrder(b);
    if (p != 0) return p;
    p = kindRank(a) - kindRank(b);
    if (p != 0) return p;
    p = seasonNo(a) - seasonNo(b);
    if (p != 0) return p;
    return YoruBrain.title(a).compareToIgnoreCase(YoruBrain.title(b));
  }

  private int releaseOrder(Anime a) {
    if (a == null) return 99999999;
    if (a.airedDate != null && a.airedDate.length() >= 4) try {
      return (
        Integer.parseInt(a.airedDate.substring(0, 4)) * 10000 +
        monthDay(a.airedDate)
      );
    } catch (Exception ignored) {}
    return a.year > 0 ? a.year * 10000 : 99999999;
  }

  private int monthDay(String d) {
    try {
      return (
        Integer.parseInt(d.substring(5, 7)) * 100 +
  ...
  private void prefetchRelated() {
    if (anime == null || anime.related.isEmpty()) return;
    ArrayList<Anime> rows = new ArrayList<>(
      anime.related.subList(0, Math.min(24, anime.related.size()))
    );
    YoruApp.app().discovery.execute(() -> {
      try {
        YoruApp.app().api.prefetchQuickDetails(rows, rows.size());
      } catch (Exception ignored) {}
    });
  }

  private String airText(String raw) {
    String v = raw == null ? "" : raw.trim();
    if (v.isEmpty() || v.equals("null")) return "дата уточняется";
    try {
      java.time.LocalDateTime t = java.time.OffsetDateTime.parse(v)
  ...
  private String episodePoster(Anime.Episode ep) {
    String p = ep == null ? "" : ApiRepository.safeUrl(ep.poster);
    if (!p.isEmpty()) return p;
    if (
      !anime.screenshots.isEmpty() && ep != null
    ) return anime.screenshots.get(
      Math.abs((int) Math.floor(ep.number) - 1) % anime.screenshots.size()
    );
    return anime.poster;
  }

  private boolean readyVoice(double number) {
    SecureStore store = YoruApp.app().store;
    String pref = store.voicePreference();
    if (
      !ApiRepository.voiceKey(pref).isEmpty() ||
      !store.favoriteVoices().isEmpty()
    ) return true;
    JSONObject h = progressCached;
    String d = ApiRepository.voiceTitle(h.optString("dubbing", ""));
    if (
      !d.isEmpty() && Math.abs(h.optDouble("episode", -9999) - number) < 0.001
    ) {
      store.voicePreference(d);
      store.rememberVoice(d);
      return true;
    }
    return false;
  }
  ...
  private void screenshots() {
    if (anime.screenshots.isEmpty()) return;
    Ui.space(body, 24);
    body.addView(Ui.text(this, "Скриншоты", 21, Ui.TEXT, true));
    Ui.space(body, 12);
    HorizontalScrollView row = new HorizontalScrollView(this);
    row.setHorizontalScrollBarEnabled(false);
    LinearLayout list = Ui.row(this);
    for (int i = 0; i < Math.min(12, anime.screenshots.size()); i++) {
      FrameLayout frame = new FrameLayout(this);
      frame.setBackground(Ui.shape(Ui.CARD, 15, this));
      frame.setClipToOutline(true);
      ImageView img = new ImageView(this);
      img.setScaleType(ImageView.ScaleType.CENTER_CROP);
      frame.addView(img, new FrameLayout.LayoutParams(-1, -1));
      String url = anime.screenshots.get(i);
      YoruApp.app().images.load(img, url, anime.key() + ":shot:" + i);
      final String open = url;
      frame.setOnClickListener(v ->
        Ui.openImage(this, open, YoruBrain.title(anime))
      );
      Ui.press(frame);
      LinearLayout.LayoutParams lp = Ui.lp(this, 194, 109);
      lp.rightMargin = Ui.dp(this, 9);
      list.addView(frame, lp);
    }
    row.addView(list);
    body.addView(row, Ui.lp(this, -1, -2));
  }

  private void trailerBlock() {
    Ui.space(body, 24);
    body.addView(Ui.text(this, "Трейлер", 21, Ui.TEXT, true));
    Ui.space(body, 12);
    String url = ApiRepository.safeUrl(anime.trailerUrl);
    if (url.isEmpty()) {
      LinearLayout c = card();
      c.addView(
        Ui.text(
          this,
          "Прямой трейлер пока не найден. Можно открыть поиск трейлера по названию.",
          12,
          Ui.MUTED,
          false
        )
      ...
      Ui.space(c, 10);
      c.addView(Ui.button(this, "Найти трейлер", false, this::trailer));
      body.addView(c);
      return;
    }
    body.addView(
      Ui.button(this, "Смотреть трейлер", false, this::trailer),
      Ui.lp(this, -1, -2)
    );
  }

  private String defaultVoiceName() {
    String title = ApiRepository.voiceTitle(
      YoruApp.app().store.voicePreference()
    );
    return title.isEmpty()
      ? YoruApp.app().store.favoriteVoices().isEmpty()
        ? "Авто"
        : "Любимые"
      : title;
  }

  private String downloadVoiceName() {
    String title = ApiRepository.voiceTitle(
      YoruApp.app().store.voicePreference()
    );
    return title.isEmpty()
  ...
  private void voiceInfoCard() {
    ...
    row.setBackground(Ui.stroke(Ui.CARD, 14, this));
    String label =
      (YoruApp.app().store.onlyPreferredVoice() &&
      !ApiRepository.voiceKey(YoruApp.app().store.voicePreference()).isEmpty()
        ? "Только озвучка: " + defaultVoiceName()
    ...
    edit.setBackground(Ui.stroke(Ui.SURFACE, 10, this));
    Ui.press(edit);
  ...
  private void chooseWatchVoice(double number, String playMode) {
    ...
    Ui.Progress wait = Ui.progress(
      this,
      "Загружаем озвучки серии " + Ui.number(number) + "…",
      true,
      () -> cancelled.set(true)
    );
    YoruApp.app().io.execute(() -> {
      try {
        WatchPack pack = watchPack(number);
        YoruApp.app().main.post(() -> {
          if (dead() || cancelled.get()) return;
          wait.dismiss();
          showWatchDialog(pack, playMode);
        });
      } catch (Exception e) {
        YoruApp.app().main.post(() -> {
          if (dead() || cancelled.get()) return;
          ...
          Ui.confirm(
            this,
            "Озвучки пока не загрузились",
            "Можно открыть авто-подбор по озвучкам или попробовать ещё раз.",
            "Открыть",
            () -> Ui.openPlayer(this, anime, "yoru", number),
            "Отмена"
          );
  ...
  private WatchPack watchPack(double number) throws Exception {
    ...
    if (ep != null) {
      String direct =
        ready != null && ready.video != null
          ? ApiRepository.sourceVoice(ready.video.source)
          : "";
      if (!direct.isEmpty() && !ep.streams.isEmpty()) voices.put(
        ApiRepository.voiceKey(direct),
        direct
      );
      for (Anime.Variant v : SourceEngine.sortVariants(ep.variants)) {
        String title = ApiRepository.voiceTitle(
          v.name.isEmpty() ? v.label() : v.name
        );
        String key = ApiRepository.voiceKey(title);
        if (!key.isEmpty() && !voices.containsKey(key)) voices.put(key, title);
        else if (
          key.isEmpty() && !ApiRepository.embed(v.url).isEmpty()
        ) unknown = true;
      }
    }
    ArrayList<String> names = new ArrayList<>(),
      values = new ArrayList<>();
    names.add("Авто · лучшая доступная");
    values.add("");
    for (String title : voices.values()) {
      names.add(title);
      values.add(title);
    }
    if (names.size() == 1 && unknown) names.set(
      0,
      "Авто · дорожка без названия"
    );
    SecureStore store = YoruApp.app().store;
    String pref = store.voicePreference();
    int selected = 0;
    for (int i = 1; i < values.size(); i++) {
      if (ApiRepository.voiceMatches(pref, values.get(i))) selected = i;
      else if (
        selected == 0 && store.voicePriority(values.get(i)) >= 0
      ) selected = i;
    }
    WatchPack pack = new WatchPack();
    pack.playback = ready;
    pack.episode = ep;
    pack.names = names.toArray(new String[0]);
    pack.values = values.toArray(new String[0]);
  ...
  private void showWatchDialog(WatchPack pack, String playMode) {
    ...
    TextView note = Ui.text(
      this,
      "Выберите озвучку и разрешение один раз. Следующие запуски этой серии откроются сразу.",
      12,
      Ui.MUTED,
      false
    );
    ...
    col.addView(note);
    Ui.space(col, 12);
    col.addView(Ui.text(this, "Озвучка", 11, Ui.MUTED, false));
    Spinner voice = new Spinner(this);
    voice.setAdapter(
      new ArrayAdapter<>(
        this,
    ...
    voice.setSelection(
      Math.max(0, Math.min(pack.selected, pack.names.length - 1))
    );
    col.addView(voice, Ui.lp(this, -1, 46));
    Ui.space(col, 10);
    col.addView(Ui.text(this, "Разрешение", 11, Ui.MUTED, false));
    ...
    quality.setSelection(qualityIndex(YoruApp.app().store.quality()));
    col.addView(quality, Ui.lp(this, -1, 46));
    Ui.space(col, 8);
    ...
    Ui.custom(
      this,
      "Серия " + Ui.number(number),
      col,
      "Смотреть",
      () -> {
        String selected = pack.values[voice.getSelectedItemPosition()];
        int[] qualities = QualityPlus.values();
        YoruApp.app().store.settings(
          qualities[quality.getSelectedItemPosition()],
      ...
      () -> {
        ...
        applyVoice(selected);
        DownloadActions.prepare(this, anime, pack.playback, number);
        render();
      },
      "Отмена"
    );
  ...
  private void applyVoice(String selected) {
    String voice = selected == null ? "" : selected;
    YoruApp.app().store.voicePreference(voice);
    if (!voice.isEmpty()) {
      YoruApp.app().store.rememberVoice(voice);
      Ui.toast(
        this,
        "Озвучка добавлена в любимые: " + ApiRepository.voiceTitle(voice)
      );
    }
  }
  ...
  private String qualityName(int q) {
    return QualityPlus.name(q);
  }

  private int qualityIndex(int q) {
    return QualityPlus.index(q);
  }
  ...
  private void watchDefaults() {
    ...
    int vi = 0;
    String cur = YoruApp.app().store.voicePreference();
    for (int i = 0; i < ApiRepository.VOICE_PREF_VALUES.length; i++) if (
      ApiRepository.voiceKey(ApiRepository.VOICE_PREF_VALUES[i]).equals(
        ApiRepository.voiceKey(cur)
      )
    ) vi = i;
    voice.setSelection(vi);
    col.addView(Ui.text(this, "Озвучка по умолчанию", 11, Ui.MUTED, false));
    col.addView(voice, Ui.lp(this, -1, 46));
    Ui.space(col, 10);
    Spinner quality = new Spinner(this);
    quality.setAdapter(
    ...
    quality.setSelection(qualityIndex(YoruApp.app().store.quality()));
    col.addView(Ui.text(this, "Разрешение по умолчанию", 11, Ui.MUTED, false));
    col.addView(quality, Ui.lp(this, -1, 46));
    Switch only = new Switch(this);
    only.setText("Открывать только выбранную озвучку");
    only.setTextColor(Ui.TEXT);
    only.setTextSize(13);
    only.setTypeface(Ui.typeface(this, false));
    only.setChecked(YoruApp.app().store.onlyPreferredVoice());
    col.addView(only, Ui.lp(this, -1, 52));
    Ui.custom(
      this,
      "Просмотр",
      col,
      ...
      () -> {
        int[] qualities = QualityPlus.values();
        String value = ApiRepository.VOICE_PREF_VALUES[
          voice.getSelectedItemPosition()
        ];
        YoruApp.app().store.voicePreference(value);
        if (!value.isEmpty()) YoruApp.app().store.rememberVoice(value);
        YoruApp.app().store.onlyPreferredVoice(
          only.isChecked() && !value.isEmpty()
        );
        YoruApp.app().store.settings(
          qualities[quality.getSelectedItemPosition()],
          YoruApp.app().store.autoNext()
        );
        Ui.toast(this, "Просмотр обновлён");
        render();
      },
      null,
      null,
