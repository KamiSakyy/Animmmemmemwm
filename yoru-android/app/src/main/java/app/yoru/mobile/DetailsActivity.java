...
public final class DetailsActivity extends Activity {
    ...
    private int maxQuality(Anime a){int q=0;if(a!=null)for(Anime.Episode e:a.episodeList){for(Integer k:e.streams.keySet())if(k!=null)q=Math.max(q,k);for(Anime.Variant v:e.variants){String text=(v.name+" "+v.displayName+" "+v.player+" "+v.url).toLowerCase(Locale.ROOT);if(text.contains("2160")||text.contains("4k")||text.contains("uhd"))q=Math.max(q,2160);else if(text.contains("1440")||text.contains("2k")||text.contains("qhd"))q=Math.max(q,1440);else if(text.contains("1080"))q=Math.max(q,1080);else if(text.contains("720"))q=Math.max(q,720);}}return q;}
    ...
    private boolean readyVoice(double number){SecureStore store=YoruApp.app().store;String pref=store.voicePreference();if(!ApiRepository.voiceKey(pref).isEmpty()||!store.favoriteVoices().isEmpty())return true;JSONObject h=progressCached;String d=ApiRepository.voiceTitle(h.optString("dubbing",""));if(!d.isEmpty()&&Math.abs(h.optDouble("episode",-9999)-number)<0.001){store.voicePreference(d);store.rememberVoice(d);return true;}return false;}
    ...
    private void applyVoice(String selected){String voice=selected==null?"":selected;YoruApp.app().store.voicePreference(voice);if(!voice.isEmpty()){YoruApp.app().store.rememberVoice(voice);Ui.toast(this,"Озвучка добавлена в любимые: "+ApiRepository.voiceTitle(voice));}}
