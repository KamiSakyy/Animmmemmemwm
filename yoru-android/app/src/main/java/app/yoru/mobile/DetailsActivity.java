...
public final class DetailsActivity extends Activity {
    ...
    private void render(){progressCached=YoruApp.app().store.progress(anime);bucketCached=YoruApp.app().store.bucket(anime);favoriteCached=YoruApp.app().store.favorite(anime);prepareEpisodeDisplay();headerSection.removeAllViews();moreSection.removeAllViews();LinearLayout body=headerSection;LinearLayout hero=Ui.row(this);hero.setGravity(Gravity.TOP);ImageView image=new ImageView(this);image.setScaleType(ImageView.ScaleType.CENTER_CROP);image.setBackground(Ui.gradient(0xff2a2135,0xff18131f,16,this));image.setClipToOutline(true);hero.addView(image,Ui.lp(this,126,184));YoruApp.app().images.load(image,anime);LinearLayout info=Ui.column(this);info.setPadding(Ui.dp(this,17),Ui.dp(this,3),0,0);TextView source=Ui.label(this,topLabel());source.setTextColor(Ui.PURPLE);info.addView(source);Ui.space(info,11);TextView title=Ui.text(this,YoruBrain.title(anime),23,Ui.TEXT,true);title.setMaxLines(6);info.addView(title);Ui.space(info,12);info.addView(Ui.text(this,anime.cardMeta()+(anime.studio.isEmpty()?"":" · "+anime.studio),11,Ui.MUTED,false));if(anime.ratingScore()>0){Ui.space(info,9);info.addView(Ui.text(this,String.format(Locale.US,"★ %.2f",anime.ratingScore()),15,Ui.PURPLE,true));}hero.addView(info,new LinearLayout.LayoutParams(0,-2,1));body.addView(hero);Ui.space(body,18);chips(body);about(body);Ui.space(body,12);body.addView(Ui.button(this,"Список серий",false,()->detailScroll.smoothScrollTo(0,episodeSection.getTop())),Ui.lp(this,-1,48));Ui.space(body,12);inlinePlayer.bind(anime);episodes();actions(moreSection);progress(moreSection);richCard(moreSection);screenshots(moreSection);trailerBlock(moreSection);related(moreSection);similar(moreSection);WallpaperStyle.refreshSurfaces(root);}
