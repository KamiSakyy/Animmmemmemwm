...
assert "versionName='0.2.0'" in manifest
...
try:
    req=urllib.request.Request('https://shikimori.io/api/graphql',data=json.dumps({'query':query}).encode(),headers={'Content-Type':'application/json','User-Agent':'YORU-VK/0.2.0 (Android; independent prototype)'})
...
(root/'handoff/YORU-VK-0.2.0-build.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n')
