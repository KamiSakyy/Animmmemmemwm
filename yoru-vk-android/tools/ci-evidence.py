...
assert "package: name='app.yoru.vk.prototype'" in manifest
assert "versionName='0.1.0'" in manifest
assert 'application-debuggable' not in manifest
result={'workflow_run':os.environ.get('GITHUB_RUN_ID'),'commit':os.environ.get('GITHUB_SHA'),'tests':suites,'manifest':manifest,'certificate':(root/'vk-cert.txt').read_text(),'apk_sha256':hashlib.sha256((root/'apk-output/YORU-VK-0.1.0-prototype.apk').read_bytes()).hexdigest(),'vk_authenticated_playback_tested':False,'device_ui_tested':False}
...
try:
    req=urllib.request.Request('https://shikimori.io/api/graphql',data=json.dumps({'query':query}).encode(),headers={'Content-Type':'application/json','User-Agent':'YORU-VK/0.1.0 (Android; independent prototype)'})
...
(root/'handoff/YORU-VK-0.1.0-build.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n')
